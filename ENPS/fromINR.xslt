<xsl:stylesheet version="1.0" encoding="utf-8" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://iptc.org/std/nar/2006-10-01/">
  ...
  <xsl:template match="/">
    ...
    <ENPSObject>
      ...
      <ENPSText>
        ...
        <xsl:for-each select="//newsItem[contains(@guid,'-video') or contains(@guid,'-photo')]">
			&#13;
			<xsl:text>&lt;mos&gt;</xsl:text>
			<xsl:text>&lt;itemID&gt;</xsl:text>2<xsl:text>&lt;/itemID&gt;</xsl:text>
			<xsl:text>&lt;objID&gt;</xsl:text><xsl:value-of select="//newsItem/contentMeta/altId[@type='foreignkey:StoryNumber']"/>  <xsl:text>&lt;/objID&gt;</xsl:text>
			...
			<xsl:text>&lt;mosAbstract&gt;</xsl:text><xsl:value-of select="contentMeta/slugline"/> : <xsl:value-of select="//newsItem/contentMeta/altId[@type='foreignkey:StoryNumber']"/><xsl:text>&lt;/mosAbstract&gt;</xsl:text>
			<xsl:text>&lt;objSlug&gt;</xsl:text><xsl:value-of select="contentMeta/slugline"/> : <xsl:value-of select="//newsItem/contentMeta/altId[@type='foreignkey:StoryNumber']"/><xsl:text>&lt;/objSlug&gt;</xsl:text>
			<xsl:text>&lt;itemSlug&gt;</xsl:text><xsl:value-of select="contentMeta/slugline"/> : <xsl:value-of select="//newsItem/contentMeta/altId[@type='foreignkey:StoryNumber']"/><xsl:text>&lt;/itemSlug&gt;</xsl:text>
			<xsl:if test="contentSet/remoteContent/@rendition = 'rnd:highRes'">
			  <xsl:text>&lt;objDur&gt;</xsl:text>
			  <xsl:value-of select="ceiling(contentSet/remoteContent[@rendition = 'rnd:highRes']/@duration * (contentSet/remoteContent/@videoframerate * 2))"/>
			  <xsl:text>&lt;/objDur&gt;</xsl:text>
			  <xsl:text>&lt;objTB&gt;</xsl:text>
			  <xsl:value-of select="ceiling(contentSet/remoteContent[@rendition = 'rnd:highRes']/@videoframerate * 2)"/>
			  <xsl:text>&lt;/objTB&gt;</xsl:text>
			</xsl:if>
			<xsl:if test="(contentMeta/icon/@href) or (contentSet/remoteContent/@href)">
			  ...
			  <xsl:for-each select="contentSet/remoteContent">
				<xsl:choose>
  ...
  <xsl:template match="contentSet" >
    ...
    <xsl:if test="//remoteContent/@rendition = 'rnd:highRes'">
      <xsl:text>&lt;objDur&gt;</xsl:text>
      <xsl:value-of select="ceiling(//remoteContent[@rendition = 'rnd:highRes']/@duration * (//remoteContent/@videoframerate * 2))"/>
      <xsl:text>&lt;/objDur&gt;</xsl:text>
      <xsl:text>&lt;objTB&gt;</xsl:text>
      <xsl:value-of select="ceiling(//remoteContent[@rendition = 'rnd:highRes']/@videoframerate * 2)"/>
      <xsl:text>&lt;/objTB&gt;</xsl:text>
